...
const client = new Client({
  ...
  puppeteer: {
    ...
    args: process.env.PPTRARGS ? process.env.PPTRARGS.split(',') : [
      '--no-sandbox',
...
const checkIsConnected = async function () {
  const isConnected = await client.getState()
    .catch(() => { logger.error('Connection error') })
  return isConnected
}
const checkRegisteredNumber = async function (number) {
  const isRegistered = await client.isRegisteredUser(number)
    .catch(() => { logger.error('Client is not connected') })
  return isRegistered
}
...
app.post('/send-group-message', [body('id').custom((value, { req }) => {
...
server.listen(port, function () {
  logger.info(`App version ${packageJson.version} running on port ${port}`)
})
